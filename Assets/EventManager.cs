...
public class EventManager : MonoBehaviour {
	...
	void Update () {
		...
		if (character.GetComponent<Collider2D>().IsTouching(granite_1.GetComponent<Collider2D>())){
			bag.GetComponent<BagScript>().addObject (granite_1);
		}

		if (character.GetComponent<Collider2D>().IsTouching(granite_2.GetComponent<Collider2D>())){
			bag.GetComponent<BagScript>().addObject (granite_2);
		}

		if (character.GetComponent<Collider2D>().IsTouching(sand_1.GetComponent<Collider2D>())){
			print ("touch sand");
			bag.GetComponent<BagScript>().addObject (sand_1);
		}

		if (character.GetComponent<Collider2D>().IsTouching(sand_2.GetComponent<Collider2D>())){
