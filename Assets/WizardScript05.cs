...
public class WizardScript05 : MonoBehaviour
{
    ...
    void Update()
    {
        if (character.transform.position.x >= -10 && character.transform.position.x <= -6)
        {
            ...
            if (GameObject.Find("bag").GetComponent<BagScript05>().sandstoneInBag)
            {
                wizardDialogAnim.SetBool("sandstone", true);
            }
            else
            {
                wizardDialogAnim.SetBool("sandstone", false);
            }
